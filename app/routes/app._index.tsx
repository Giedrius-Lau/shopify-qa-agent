...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return { liveBase: `https://${session.shop}`, recentScans };
};

export const action = async ({ request }: ActionFunctionArgs): Promise<ActionData> => {
  const { session } = await authenticate.admin(request);
  try {
    const body = await request.json() as { pagePath?: unknown; previewUrl?: unknown; storePassword?: unknown; viewports?: unknown };
    ...
    if (typeof body.previewUrl !== "string" || !body.previewUrl.startsWith("https://")) return { ok: false, error: "Enter the complete HTTPS preview theme URL." };
    ...
    const result = await runEmbeddedComparison({ shop: session.shop, pagePath: body.pagePath, previewUrl: body.previewUrl, viewports, storefrontPassword: typeof body.storePassword === "string" ? body.storePassword : undefined });
...
export default function Index() {
  const { liveBase, recentScans } = useLoaderData<typeof loader>();
  ...
  const submit = () => fetcher.submit({ pagePath, previewUrl, storePassword, viewports }, { method: "POST", encType: "application/json" });

  return <s-page heading="Shopify QA Agent"><div className="qa-main"><header className="hero"><span className="brand">THEME QA</span><h1>Compare live against preview.</h1><p>The installed shop is selected automatically. Enter a page path and the unpublished theme preview link.</p></header>
    <section className="scan-form"><div className="field-grid"><label><span>Installed storefront</span><input value={liveBase} readOnly/></label><label><span>Page path</span><input value={pagePath} onChange={(event) => setPagePath(event.target.value)} placeholder="/products/example"/></label><label><span>Preview theme URL</span><input type="url" required value={previewUrl} onChange={(event) => setPreviewUrl(event.target.value)} placeholder="https://...shopifypreview.com"/></label><label><span>Storefront password <em>optional</em></span><input type="password" autoComplete="off" value={storePassword} onChange={(event) => setStorePassword(event.target.value)} placeholder="Used only if Shopify asks"/></label></div><div className="form-footer"><fieldset><legend>Viewports</legend><label className="check"><input type="checkbox" checked={viewports.includes("desktop")} onChange={() => toggleViewport("desktop")}/> Desktop</label><label className="check"><input type="checkbox" checked={viewports.includes("mobile")} onChange={() => toggleViewport("mobile")}/> Mobile</label></fieldset><button disabled={loading || !previewUrl || viewports.length === 0} onClick={submit}>{loading ? "Scanning…" : "Run comparison"}</button></div><p className="privacy">Preview tokens and storefront passwords are never persisted or returned.</p></section>
