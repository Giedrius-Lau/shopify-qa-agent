...
function VisualComparison({ live, preview, themesAreCodeIdentical }: { live: PageScanResult; preview: PageScanResult; themesAreCodeIdentical: boolean }) {
  ...
  const highlights = themesAreCodeIdentical ? [] : preview.sections.flatMap((section) => { const change = changed.get(sectionKey(section.id, section.name)); return change && section.bounds && preview.pageHeight ? [{ ...change, bounds: section.bounds, key: section.id }] : []; });
  return <section className="visual-comparison"><div className="visual-heading"><div><span className="eyebrow">Page preview</span><h3>Compare the two themes</h3><p>{themesAreCodeIdentical ? "No sections are highlighted because the Shopify theme files match exactly." : "Updated sections are outlined on the unpublished theme."}</p></div>{highlights.length > 0 && <div className="highlight-legend"><span className="changed">Changed</span><span className="added">Added</span></div>}</div><div className="screenshot-grid"><figure><figcaption><strong>Baseline theme</strong><span>Before</span></figcaption><img src={live.screenshotPath} alt="Baseline theme screenshot" loading="eager"/></figure><figure><figcaption><strong>Unpublished theme</strong><span>{themesAreCodeIdentical ? "Matching files" : `After · ${highlights.length} highlighted`}</span></figcaption><div className="annotated-shot"><img src={preview.screenshotPath} alt="Unpublished theme screenshot with changed sections highlighted" loading="eager"/>{highlights.map((item) => <span className={`section-highlight ${item.kind}`} key={item.key} style={{ left: `${item.bounds.x / preview.viewportSize.width * 100}%`, top: `${item.bounds.y / preview.pageHeight! * 100}%`, width: `${item.bounds.width / preview.viewportSize.width * 100}%`, height: `${item.bounds.height / preview.pageHeight! * 100}%` }}><b>{item.name}</b></span>)}</div></figure></div></section>;
}

function DeltaList({ title, tone, issues }: { title: string; tone: "added" | "baseline-only"; issues: QaIssue[] }) {
...
function ThemeDelta({ live, preview, themesAreCodeIdentical }: { live: PageScanResult; preview: PageScanResult; themesAreCodeIdentical: boolean }) {
  ...
  if (themesAreCodeIdentical) return <section className="delta-card"><div className="delta-summary"><div><span className="eyebrow">QA summary</span><h3>No deterministic theme changes found</h3><p>The Shopify theme files are identical. One-off browser differences are treated as runtime variation, not as improvements or regressions.</p></div></div></section>;
  return <section className="delta-card"><div className="delta-summary"><div><span className="eyebrow">QA summary</span><h3>What changed in the unpublished theme</h3></div><div className="delta-stats"><span><strong>{new Set(issueDiff.added.map((issue) => issue.rule)).size}</strong> only in unpublished</span><span><strong>{issueDiff.resolved.length}</strong> only in baseline</span></div></div>
    ...
    <div className="delta-grid"><DeltaList title="Only in unpublished theme" tone="added" issues={issueDiff.added}/><DeltaList title="Only in baseline theme" tone="baseline-only" issues={issueDiff.resolved}/></div></section>;
}
...
function ReleaseSummary({ result }: { result: EmbeddedScanResult }) {
  const summary = buildReportSummary(result);
  return <section className={`release-summary ${summary.decision}`}><div className="release-copy"><span className="eyebrow">Release recommendation</span><h2>{summary.headline}</h2><p>{summary.description}</p></div><div className="release-metrics"><span><strong>{summary.pagesCompared}</strong> pages</span><span><strong>{summary.newConcerns}</strong> new concerns</span><span><strong>{summary.changedSections}</strong> changed sections</span><span><strong>{summary.changedFiles}</strong> changed files</span></div>{summary.priorities.length > 0 && <div className="release-priorities"><h3>Check these first</h3>{summary.priorities.map((item, index) => <article key={`${item.page}:${item.message}:${index}`}><span className={`severity-label ${item.severity}`}>{item.severity}</span><div><strong>{item.message}</strong><small>{item.page}{item.page.startsWith("/") ? ` · ${item.viewport}` : ""}</small></div></article>)}</div>}</section>;
}

function AiExplanation({ explanation }: { explanation: NonNullable<EmbeddedScanResult["aiExplanation"]> }) {
  return <section className="ai-explanation"><div><span className="eyebrow">AI explanation</span><h3>{explanation.summary}</h3><p>{explanation.releaseRationale}</p></div>{explanation.actions.length > 0 && <div className="ai-actions"><h4>Recommended next actions</h4>{explanation.actions.map((action, index) => <article key={`${action.title}:${index}`}><strong>{action.title}</strong><p>{action.reason}</p>{action.evidenceIds.length > 0 && <small>Evidence: {action.evidenceIds.join(", ")}</small>}</article>)}</div>}<small className="ai-disclosure">Generated only from this scan’s deterministic evidence.</small></section>;
}

function AiExplanationUnavailable({ message }: { message: string }) {
  return <section className="ai-explanation unavailable" role="status"><div><span className="eyebrow">AI explanation unavailable</span><h3>The QA report is ready, but its optional AI summary was not created.</h3><p>{message}</p></div></section>;
}
...
export default function Index() {
  ...
  const sortedThemes = [...themes].sort((left, right) => Number(right.role === "MAIN") - Number(left.role === "MAIN"));
  const baselineTheme = themes.find((theme) => theme.id === baselineThemeId);
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch(`/app/scan-stream${window.location.search}`, { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ pagePaths, baselineThemeId, comparisonThemeId, storePassword, viewports, codeOnly, explainWithAi }) });
  ...
  return <s-page heading="Shopify QA Agent"><div className="qa-main"><header className="hero"><span className="brand">THEME QA</span><h1>Compare one theme against another.</h1><p>Select a baseline theme and an unpublished theme from the installed store. No preview link is required.</p></header>
    <section className="scan-form"><div className="field-grid"><label><span className="field-label">Baseline theme {baselineTheme?.role === "MAIN" && <b className="live-badge">LIVE</b>}</span><select value={baselineThemeId} onChange={(event) => selectBaseline(event.target.value)}>{sortedThemes.map((theme) => <option className={theme.role === "MAIN" ? "live-theme-option" : undefined} key={theme.id} value={theme.id} disabled={theme.processing}>{theme.name}{theme.role === "MAIN" ? " — LIVE" : ` — ${theme.role.toLowerCase()}`}{theme.processing ? " (processing)" : ""}</option>)}</select></label><label><span>Unpublished comparison theme</span><select required value={comparisonThemeId} onChange={(event) => setComparisonThemeId(event.target.value)}><option value="">Select an unpublished theme…</option>{comparisonThemes.map((theme) => <option key={theme.id} value={theme.id} disabled={theme.processing}>{theme.name}{theme.processing ? " (processing)" : ""}</option>)}</select></label><label><span>Installed storefront</span><input value={liveBase} readOnly/></label>{!codeOnly && <label><span>Storefront password <em>optional</em></span><input type="password" autoComplete="off" value={storePassword} onChange={(event) => setStorePassword(event.target.value)} placeholder="Used only if Shopify asks"/></label>}</div>
      ...
      <div className="scan-mode"><input id="code-only" type="checkbox" checked={codeOnly} onChange={(event) => setCodeOnly(event.target.checked)}/><label htmlFor="code-only"><strong>Code-only scan</strong><small>Skip Chromium, screenshots, accessibility, links, and visual checks.</small></label></div><div className="scan-mode"><input id="explain-with-ai" type="checkbox" checked={explainWithAi} onChange={(event) => setExplainWithAi(event.target.checked)}/><label htmlFor="explain-with-ai"><strong>Explain this report with AI</strong><small>Send only the deterministic summary, finding messages, affected locations, and changed-file names to OpenAI. Screenshots, passwords, theme source, and storefront URLs are not sent.</small></label></div><div className="form-footer">{!codeOnly && <fieldset><legend>Viewports</legend><label className="check"><input type="checkbox" checked={viewports.includes("desktop")} onChange={() => toggleViewport("desktop")}/> Desktop</label><label className="check"><input type="checkbox" checked={viewports.includes("mobile")} onChange={() => toggleViewport("mobile")}/> Mobile</label></fieldset>}<button disabled={loading || !baselineThemeId || !comparisonThemeId || (!codeOnly && (viewports.length === 0 || pagePaths.length === 0))} onClick={submit}>{loading ? "Scanning…" : codeOnly ? "Compare code" : `Scan ${pagePaths.length} page${pagePaths.length === 1 ? "" : "s"}`}</button></div><p className="privacy">Theme IDs and storefront passwords are never exposed in reports.</p></section>
    ...
    {result && <section className="results"><div className="section-heading"><div><span className="eyebrow">Scan complete</span><h2>Theme comparison</h2></div></div>{result.aiExplanation && <AiExplanation explanation={result.aiExplanation}/>} {result.aiExplanationRequested && !result.aiExplanation && <AiExplanationUnavailable message={result.aiExplanationError ?? "The AI service did not return an explanation."}/>}<ReleaseSummary result={result}/>{result.codeAccessibilityIssues && <CodeAccessibility issues={result.codeAccessibilityIssues}/>} {result.codeChanges && <CodeChanges changes={result.codeChanges}/>} {pageGroups.map((group) => <section className="page-result" key={group.pathname}><div className="page-result-heading"><div><span className="eyebrow">Storefront page</span><h2>{pageLabel(group.pathname)}</h2></div><code>{group.pathname}</code></div>{group.pairs.map(({ viewport, live, preview }) => preview && <section key={`${group.pathname}:${viewport}`} className="viewport-group"><h2>{viewport === "desktop" ? "Desktop view" : "Mobile view"}</h2><VisualComparison live={live} preview={preview} themesAreCodeIdentical={themesAreCodeIdentical}/><ThemeDelta live={live} preview={preview} themesAreCodeIdentical={themesAreCodeIdentical}/></section>)}</section>)}</section>}
